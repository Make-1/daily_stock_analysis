...
def format_topn_markdown(items: Sequence[ScreenedStock], top_n: int) -> str:
    ...
    lines.append("")
    lines.append("## 排行速览")
    lines.append("| # | 代码 | 评分 | 信号 | 趋势 | 现价 |")
    lines.append("| --- | --- | --- | --- | --- | --- |")
    for idx, it in enumerate(items, start=1):
        lines.append(
            f"| {idx} | **{it.code}** | {it.signal_score} | {it.buy_signal} | {it.trend_status} | {it.current_price:.2f} |"
        )

    lines.append("")
    lines.append("## 指标详情")
    for idx, it in enumerate(items, start=1):
        lines.append(f"### {idx}. {it.code} （评分 {it.signal_score} · {it.buy_signal}）")
        # 均线 + 乖离
        lines.append(
